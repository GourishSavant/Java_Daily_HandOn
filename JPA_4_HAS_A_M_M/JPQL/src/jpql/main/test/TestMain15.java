...
public class TestMain15 {
	...
	public static void main(String[] args) {
		...
		// Query on age greater than 20
		ParameterExpression<Integer> pe = cb.parameter(Integer.class);
		cq.select(root).where(cb.gt(root.get("age"), pe));
		
		// Execute the query
		TypedQuery<User> query = em.createQuery(cq);
		query.setParameter(pe, 20);
